...
void sendBluetooth(BluetoothServer *bt)
{
  while (bt->isConnected()){
	...
	if(can_Ok==1){
		...
		positionVoiture *CarPosition = linkPositionVoiture();
		data_paquet_t position;
		position.floatMessage[0] = (float)CarPosition->dist;
		position.floatMessage[1] = CarPosition->percentageSide;

		int32_t  profondeur = (int32_t)((position.intMessage[1]<<16)&0xFFFF0000)|((int32_t)position.intMessage[0]&0x0000FFFF);
		int32_t  percent_cote = (int32_t)((position.intMessage[3]<<16)&0xFFFF0000)|((int32_t)position.intMessage[2]&0x0000FFFF);


		Messages msg_posVoiture = Messages(0x00, 0x02, CarPosition->side+1, profondeur, percent_cote );
		cout << Messages::encode(msg_posVoiture) << endl ;
		bt->sendMsg(Messages::encode(msg_posVoiture));
		/*
		bt->sendMsg("[DBG] dist"+std::to_string(CarPosition->dist));
		bt->sendMsg("[DBG] side"+std::to_string(CarPosition->side));
		bt->sendMsg("[DBG] per"+std::to_string(CarPosition->percentageSide));
		*/
	}
  }
}
...
int main()
{
	...
	std::thread imageProcessing(threadTraitement);
